...
\chapter{Register územných adries a nehnuteľností} 
\label{struktura}
V polovici roku 2012 bol úspešne spustený systém základných registrov veřejné správy ČR. Jeden zo štyroch základných registrov je i register územnej identifikácie, adries a nehnuteľností (RÚIAN)\cite{Ruian}. RÚIAN je verejný zoznam, ktorý umožňuje uživateľom z verejnej, ale i komerčnej a akademickej sféry, ďiaľkový prístup cez internet.
\section{Typy poskytovania údajov z registra územnej identifikácie adries a nehnuteľností}
...
Spôsob umožňuje nazerať a získávat data základného registru RÚIAN\cite{Ruian} a tiež niektoré data editačného agendového informačného systému územnej identifikácie (ISÚI)\cite{Isui} a informačného systému katastru nehnuteľností (ISKN)\cite{Iksn} pomocou aplikácie verejného diaľkového prístupu\cite{VDP}.
Pre prístup do aplikácie VDP nie je potrebná žiadna registrácia. Poskytované data z VDP sú zadarmo. Data poskytované prostredníctvom VDP nie sú referenčné, majú iba informačný charakter.
Podorbnej analýze VDP sa venuje kapitola 3 \ref{vdp}
\subsection{Webové služby základných registrov}
V rámci Informačného systému základných registrov\cite{Iszr} fungujú štyri základné registre verejnej správy (\bf register osôb, register obyvateľov, RUIAN, register práv a povinností\rm).
Webové služby ISZR slúžia pre komunikáciu a výmenu dat medzi registrovanými agendovými informačnými systémami\cite{Ais} verejnej správy so základnými registrami.
Služby sú publikované na vonkajšom rozhraní systémov základných registrov.
\subsection{Výmenný formát RÚIAN}
Spôsob umožňuje získavať dáta z RÚIAN pomocou výmenného formátu RÚIAN\cite{Vfr}. Jedná sa o súbory vo formáte GML 3.2.1. Tieto súbory je možné sťahovať nasledujúcimi spôsobmi: 
\begin{itemize}
	\item{prostredníctvom aplikácie VDP}
	\item{z internetových (URL) adries vrátených službami ISZR \it{ruianSouboryDat} a \it{ruianSouboryZmen} – dostupné iba orgánom
      státní správy a samosprávy (nie sú voľne prístupné širokej verejnosti)}
\end{itemize}
Analýze VFR sa ďalej podrobne venuje kapitola 4 \ref{vfr}
\subsection{Súbory s hodnotami oddelenými čiarkami}
Súbory obsahúju data RÚIAN vo formáte CSV a sú generované raz za mesiac. Pre každú obec existuje jeden samostatný CSV súbor\cite{Csv}.

Okrem zvýšenia efektivity štátnej správy, keď úradník nemusí zisťovať, ktoré dáta śú aktuální a správné, dochádza tiež ku zrýchleniu procesu vybavenia žiadostí, teda kú zníženiu byrokratickej záťaže. Ďaleko menej času strávite za prepážkou úradu. Štát pracuje čím ďalej, tím efektivnejšie, vy ušetríte čas i peniaze.
