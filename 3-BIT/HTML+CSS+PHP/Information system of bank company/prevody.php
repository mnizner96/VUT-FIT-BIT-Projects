...
<div class="wrapper row2tmp" style="height:640px;">
  ...
  <?php 
  $log = $_SESSION['Login']; 
  $query = "SELECT * FROM operace WHERE id_Klient = '".mysql_real_escape_string($log)."' AND PrevadzanaCiastka IS NOT NULL";
  $result = @mysql_query($query,$db);
  if (!($result))
  ...
  while ($account = mysql_fetch_array($result, MYSQL_ASSOC)) {
   ...
   <tr>
    ...
    <td> <?php echo $account['CisloUctuPrijemcu'];?> </td>
