...
<div class="wrapper row2tmp" style="height:640px;">
  ...
  <?php 
  $log = $_SESSION['Login']; 
  $query = "SELECT * FROM operace WHERE id_Klient = '".mysql_real_escape_string($log)."' AND (MONTH(DatumVykonania) = '".mysql_real_escape_string($_SESSION['month'])."' AND YEAR(DatumVykonania) = '".mysql_real_escape_string($_SESSION['year'])."')";
  $result = @mysql_query($query,$db);
  if (!($result))
    echo "fasa";
  while ($account = mysql_fetch_array($result, MYSQL_ASSOC)) {
    ?>
   <tr>
    <td> <?php echo $account['DatumVykonania'];?> </td>
    <td> <?php echo $account['id_Operace'];?> </td>
    <?php if(!(is_null($account['PrevadzanaCiastka']))) {
    ?>
    <td> <?php echo "Prevod";?> </td>
    <td> <?php echo $account['id_ucet']."(".$account['CisloUctuPrijemcu'].")";?> </td>
    <td> <?php echo "InternetBanking";?> </td>
    <td> <?php echo $account['PrevadzanaCiastka'] ?> </td>
    <td> <?php echo $account['PrevadzanaMena'] ?> </td>

    <?php } 
    else if (!(is_null($account['VkladanaCiastka']))) { ?>
    <td> <?php echo "Vklad";?> </td>
    <td> <?php echo $account['id_ucet'];?> </td>
    <td> <?php echo $account["MiestoVkladu"];?> </td>
    <td> <?php echo $account['VkladanaCiastka'] ?> </td>
    <td> <?php echo $account['VkladanaMena'] ?> </td>

    <?php } 
    else {?>
    <td> <?php echo "Vyber";?> </td>
    <td> <?php echo $account['id_ucet'];?> </td>
    <td> <?php echo $account["MiestoVyberu"];?> </td>
    <td> <?php echo $account['VyberovaCiastka'] ?> </td>
    <td> <?php echo $account['VyberovaMena'] ?> </td>
    </tr> 
