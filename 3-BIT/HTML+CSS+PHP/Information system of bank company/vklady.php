...
<div class="wrapper row2tmp" style="height:640px;">
  ...
  <tr>
    <th>Dátum</th>
    <th>Číslo operácie</th>
    <th>Číslo Vášho účtu</th>
    <th>Meno Vykonávateľa</th> 
    <th>Suma</th>
    <th>Mena</th>
    <th>Miesto Vkladu</th>
  </tr>
  <?php 
  $log = $_SESSION['Login']; 
  $query = "SELECT * FROM operace WHERE id_Klient = '".mysql_real_escape_string($log)."' AND MiestoVkladu IS NOT NULL";
  $result = @mysql_query($query,$db);
  if (!($result))
    echo "<script type='text/javascript'>alert('Select zlyhal!')</script>";
  while ($account = mysql_fetch_array($result, MYSQL_ASSOC)) {
   ...
   <tr>
    ...
    <td> <?php echo $account['MenoVykonavatela'];?> </td>
