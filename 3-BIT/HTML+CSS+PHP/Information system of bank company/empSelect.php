...
<div class="wrapper row2">
  <div id="container" class="clear">
    ...
    <?php 
      ...
      ?> <tr><td></td><td><font size="3" color="red">Prosim vyplnte pozadovany udaj</font></td></tr>
      <?php }  ?>
    <tr><td>Výberová čiastka:</td><td><input type="text" name="VC" placeholder="EUR"></td></tr>
    <tr><td>Miesto výberu:</td><td><input type="text" name="MV" value = <?php if(isset($_POST['MV'])) { echo $_POST['MV']; }?>></td></tr>
    <?php 
...
<?php
  ...
  if ((!(empty($_POST["CU"]))) && (!(empty($_POST["MK"]))) && (!(empty($_POST["MZ"]))) && (!(empty($_POST["VC"]))) && (!(empty($_POST["MV"])))) {
    ...
    if((!($result)) || (($resultData["uctovny_zostatok"] - $_POST["VC"]) < 0))
        echo "<script type='text/javascript'>alert('Nesprávny účet alebo nedostatok peňazí!')</script>";
    else{
        ...
        if(!($result))
         echo "<script type='text/javascript'>alert('Nesprávny zamestnanec!')</script>";
          ...
          if(!($result))
           ...
           $date = date('Y-m-d');
           $query = "INSERT INTO operace(`MenoVykonavatela`, `VyberovaCiastka`, `MiestoVyberu` ,`id_Klient`, `id_ucet`, `DatumVykonania`) VALUES ('".mysql_real_escape_string($_POST["MZ"])."','".mysql_real_escape_string($_POST["VC"])."','".mysql_real_escape_string($_POST["MV"])."','".mysql_real_escape_string($_POST["MK"])."','".mysql_real_escape_string($_POST["CU"])."', '".mysql_real_escape_string($date)."');";
            $result = @mysql_query($query,$db);
            if(!($result)){
               echo "<script type='text/javascript'>alert('Nesprávne údaje vkladanie sa nepodarilo!')</script>";
            }
            else {
               echo "<script type='text/javascript'>alert('Operácia prebehla úspešne'); window.location.href='empSelect.php';</script>";
            }
          } 
        } 
      } 
