...
** funkci stackEmpty.
*/
      if (stackEmpty(s) != 0) //pokial je zasobnik prazdny tak vola funkciu stackError
      {
      	  stackError(SERR_TOP);
      	  return;
      }
...
*/
      ...
      s->top = s->top + 1; //navysenie kapacity zasobnika
      s->arr[s->top] = c;  //vloazenie noveho znaku na vrchol zasobnika
}
