...
**/
     ...
     tBTNodePtr kde = NULL;
     if (pom == NULL) //overenie ci strom  existuje
     {
          found = false;
          kde = NULL;
     }
     else  //ak strom existuje, hladanie uzlu s hodnotou Content 
     {
          do
          {
               kde = pom;
               if (pom->Cont > Content)
                  pom = pom->LPtr;
               else if (pom->Cont < Content)
                  pom = pom->RPtr;
               else
                  found = true;  //ak sa nasiel uzol s hodnotou Content
          }while ((found != true) && (pom != NULL));
     }
     if (found == true)
         kde->Cont = Content;  //prepisanie honoty najdeneho uzlu novou hodnotou
     else //ak uzol s hodnotou Content neexsistuje, vytvorenie noveho uzlu
     {
         tBTNodePtr pom2 = NULL; 
         if ((pom2 = malloc(sizeof(struct tBTNode))) == NULL) //alokacia miesta pre novy uzol
            return;
         if (kde == NULL)   //pridanie noveho uzlu
         {
              *RootPtr = pom2;
         ...
         else if (kde->Cont > Content) //novy uzol sa prida ako lavy list
         {   
              kde->LPtr = pom2;
              pom2->Cont = Content;
              pom2->LPtr = NULL;
         ...
         else                         // novy uzol sa prida ako pravy list
         {
              kde->RPtr = pom2;
              pom2->Cont = Content;
              pom2->LPtr = NULL;
...
**/
	 while (ptr != NULL)
	 {
	 	 BTWorkOut (ptr);  //spracovanie navstiveneho uzlu
	 	 SPushP (Stack, ptr);   //ulozenie uzlu na vrchol zasobnika
	 	 ptr = ptr->LPtr;       //posunutie sa dolava na dalsi uzol
	 }	 
}
...
**/
	 ...
	 while (SEmptyP (&s) != 1)   //pokial zasobnik nie je prazdny
	 {
	 	RootPtr = STopPopP(&s);    //ziskanie uzlu s vrcholu zasobniku a zaroven odstranenie uzlu s vrcholu zasobniku
...
**/
     ...
     {
         SPushP (StackP, ptr);
         SPushB (StackB, true); //ulozenie bool hodnoty na vrchol zasobnika
         ptr = ptr->LPtr;       //posunutie sa dolava na dalsi uzol
     }
 }
...
**/
     ...
     tStackP sp;
     tStackB sb;
     SInitP (&sp);   //inicializacia zasobnikov
     SInitB (&sb);
     Leftmost_Postorder (RootPtr, &sp, &sb);
     while (SEmptyP (&sp) != 1)  //pokial zasobnik nie je prazdny
     {
         RootPtr = STopPopP (&sp); //ziskanie uzlu s vrcholu zasobnika 
         SPushP (&sp, RootPtr);
         left = STopPopB (&sb);
         if (left == true) //lava strana stromu
         {
             SPushB (&sb, false);
             Leftmost_Postorder(RootPtr->RPtr, &sp, &sb);
         }
         else    //prava strana stromu
         {
             STopPopP(&sp);
             BTWorkOut(RootPtr);
         }
...
**/
     ...
     if (*RootPtr != NULL)
     {
         ...
         do
         {
             ...
             {
                 if (SEmptyP(&s) == 0) //ak zasobnik nie je prazdny, ziskanie uzlu s vrcholu zasobnika
                    *RootPtr = STopPopP(&s);
             }
             else //ak uzol existuje
             {
                 if ((*RootPtr)->RPtr != NULL) //ak existuje prava vetva uzlu
                    SPushP(&s, (*RootPtr)->RPtr);  //ulozenie pravej vetvy uzlu na zasobnik
             }
             pom = *RootPtr; //zrusenie aktualneho uzlu
            *RootPtr = (*RootPtr)->LPtr;    
             free(pom);

         }while((*RootPtr != NULL) || (SEmptyP(&s) != 1));
     }
 }
